...
const EventItem = ({ event }) => {
  return (
    <div className={style.eventItem}>
      ...
      <div className={`${style.accent} m-5 flex-[3] truncate`}>
        {event.from}
      </div>
      <div className={`${style.accent} m-5 flex-[3] truncate`}>{event.to}</div>
      <div className={`${style.accent} m-5 flex-[2]`}>{event.date}</div>
    </div>
  )
}
